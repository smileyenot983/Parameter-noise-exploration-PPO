...
class CartPoleSwingUpV2(CartPoleSwingUpEnv):
    """CartPoleSwingUp with sparse reward. Reward is only given for pendulum up position"""

    @staticmethod
    def _reward_fn(state, action, next_state):
        if np.pi/3 <=next_state.theta <= 2*np.pi/3:
            return 1
        else:            
            return 0
