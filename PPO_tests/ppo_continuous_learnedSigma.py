...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var):
        ...
        self.action_layer = nn.Sequential(
            ...
            #network will not only output mean value but also std
            nn.Linear(n_latent_var, action_dim*2),
            # nn.Softmax()
    ...
    def act(self, state, memory):
        ...
        # get mean of action
        out = self.action_layer(state)

        action_mean = out[0:self.action_dim].detach()
        action_std = float(out[1*self.action_dim:].data)

        self.action_var = torch.full((self.action_dim,), action_std**2).to(device)
        # print(self.action_var)
        cov_mat = torch.diag(self.action_var, ).to(device)

        # multidimensional normal distribution with mean=action_mean, variance=cov_mat
        action_distribution = MultivariateNormal(action_mean.data, cov_mat)
    ...
    def evaluate(self, state, action):
        ...
        action_mean = out[:,0].detach()
        action_std = out[:,1].data

        action_var = action_std**2


        # variance of each action
        # self.action_var = torch.full((self.action_dim,), action_std ** 2).to(device)
        # self.action_var = self.action_var.expand_as(action_mean)
...
class PPO:
    def __init__(self, state_dim, action_dim, n_latent_var, K_epochs):
        ...
        self.policy_new = ActorCritic(state_dim, action_dim, n_latent_var).to(device)
...
def train(max_episodes=5000, add_parameter_noise=False,pretrained=False):
    ...
    # parameter noise implementation for contextual bandits i found on the internet:
    # https: // github.com / tensorflow / models / blob / 0
    # 874
    # d1305784ee6f037ea9312d47624d2e5da788 / research / deep_contextual_bandits / bandits / algorithms / parameter_noise_sampling.py
    ...
    policy_perturbed = ActorCritic(state_dim, action_dim, n_latent_var).to(device)
...
rewards = train(add_parameter_noise=True, max_episodes=5000,pretrained=False)
